...
function getFakeMessageContainer($message){
    $class = "";
    $avater = "";
    $sent = "";
    if( $message->type == 'received' ){
        // $class = "r";
        $sent = '<div class="chat-message-left pb-4">
        <div>
            <img src="'. GetMedia($message->from_avater) .'"
                class="rounded-circle mr-1" alt="Chris Wood" width="30" height="30" alt="'. $message->to_name .'" title="'. $message->to_name .'">
        </div>
        ...
        $sent = '<div class="chat-message-right pb-4">
        <div>
            <div class="text-muted small text-nowrap mt-2"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="#03A9F4" d="M0.41,13.41L6,19L7.41,17.58L1.83,12M22.24,5.58L11.66,16.17L7.5,12L6.07,13.41L11.66,19L23.66,7M18,7L16.59,5.58L10.24,11.93L11.66,13.34L18,7Z" /></svg></div>
        </div>
        <div class="flex-shrink-1 bg-light rounded py-2 px-3 mr-3">
            <div class="font-weight-bold mb-1">You</div>
            Lorem ipsum dolor sit amet, vis erat denique in, dicunt prodesset te vix.
...
function generate_fake_chat_messages_convirsation($user_id,$to,$offset = 0,$show_unreadline = true,$lastmsg = 0,$prev=false){
    global $db,$console_log;
    ...
    if( !empty( $chat_messages ) ) {
        ...
        if ($chat_messages) {
            ...
            foreach ($chat_messages as $key => $value) {
                ...
                // if (isset($chat_messages[$key + 1])) {
                //     if ($chat_messages[$key + 1]['to'] !== $value['to']) {
                //         $chat_messages[$key]['container'] = getFakeMessageContainer(ToObject($chat_messages[$key]));
                //         $MessageContainer = getFakeMessageContainer(ToObject($chat_messages[$key]));
                //     }
                //     if ($chat_messages[$key]['seen'] === 0) {
                //         $is_unread_show = true;
                //         if ($_msg) {
