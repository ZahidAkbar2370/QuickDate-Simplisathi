...
Class Chat extends Aj {
    ...
    function send_message_fk_admin() {
        global $db, $config;
        $html_return = '';
        if (self::ActiveUser() == NULL) {
            return array(
    ...
    function send_message() {
        ...
        if ($to > 0) {
            ...
            if ($saved) {
                // cut creditPerMessage
                $this->creditCutPerMessage();
    ...
    function creditCutPerMessage(){
        global $db, $config;
        $user_id = self::ActiveUser()->id;
        $reminingBalance = self::ActiveUser()->balance - $config->cost_per_message;
        $db->query("update users set balance='$reminingBalance' where id = '$user_id'");
    }
