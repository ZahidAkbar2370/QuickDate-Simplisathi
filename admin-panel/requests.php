...
$usersData .=  '<div class="alert alert-soft-info  align-items-center" id="no-new-messages" role="alert" style="margin: 10px; text-align: center;">
...
$usersData .= '<div class="card p-2 mb-2 ml-2"  data-conv="15163161301516316000" data-chatid="782" data-uid="<?=$v->sender_id;?>" data-search-bind="'.$v->senderName.'" data-cid="'.$v->receiver_id.'" data-name="'.$v->receiverName.'" data-id="'.$v->sender_id.'" onclick="viewChat('.$v->sender_id.','.$v->receiver_id.');" data-photo="'.$sender_image .'" style="cursor: pointer;">

<div class="list-group-item list-group-item-action border-0">
                        <div class="d-flex align-items-start">';

         if($sender_image) { 
            $usersData .= '<img src="'.$sender_image.'" alt="Avatar" class="rounded-circle mr-1" width="30" height="30">'; }
                         else { 
                            $usersData .= '<img src="'.$site_url.' ?>/upload/photos/d-avatar.jpg" alt="Avatar" class="rounded-circle mr-1" width="30" height="30">';
 } 

 $usersData .= '<div class="flex-grow-1 ml-3">
                            From: '.$v->senderName . '
                            </div>
                        </div>
                    ...
                    <div class="list-group-item list-group-item-action border-0">
                       ...
                       if($receiver_image) { 
                        $usersData .=  '<img src="'.$receiver_image .'" alt="Avatar" class="rounded-circle mr-1" width="30" height="30"> ';
                         } else {
                            $usersData .= '<img src="'.$site_url .'/upload/photos/d-avatar.jpg" alt="Avatar" class="rounded-circle mr-1" width="30" height="30">';
 }
...
//    $data =  include "pages/manage-fakechat/show_chat_user.phtml";
...
if($f=='getchat'){
	...
	$receiverid = $_GET['receiverid'];
	 ...
	 $data['status']=(generate_fake_chat_messages_convirsation((int) $senderid, (int) $receiverid, 0, true))?200 : 0;
	 $data['chat']=generate_fake_chat_messages_convirsation((int) $senderid, (int) $receiverid, 0, true);	
